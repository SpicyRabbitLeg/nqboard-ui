<template>
        ...
        <el-row>
            <el-col>
                <el-form-item :label="$t('device.serverUri')" prop="serverUri">
                    <el-input v-model="driverForm.serverUri" :placeholder="$t('device.inputServerUri')" />
                </el-form-item>
                ...
                <el-form-item :label="$t('device.keepAliveInterval')" prop="keepAliveInterval">
                    <el-input v-model="driverForm.keepAliveInterval" :placeholder="$t('device.inputKeepAliveInterval')" />
                </el-form-item>

                <el-form-item :label="$t('device.connectionTimeout')" prop="connectionTimeout">
                    <el-input v-model="driverForm.connectionTimeout" :placeholder="$t('device.inputConnectionTimeout')" />
                </el-form-item>
            </el-col>
        </el-row>
    </el-form>
    ...
    <el-form v-if="active == 2" ref="pointFormRef" :rules="pointRules" :model="pointForm" v-loading="loading">
        <el-row :gutter="24">
             <el-col :span="6">
                <el-form-item :label="$t('device.label')" prop="label">
                    <el-input v-model="pointForm.label" :placeholder="$t('device.inputLabelTip')" />
                </el-form-item>
            </el-col>
        </el-row>
    </el-form>
        ...
        <el-col :span="6" class="card-item" v-for="item in pointList" :key="item.id">
            <el-card shadow="hover" @click="change(item.id, item)"
                ...
                <div>
                    ...
                    <p>{{ $t('device.label') }}：{{ item.point?.pointInfo.label }}</p>
                </div>
            </el-card>
        </el-col>
    </el-row>
...
const driverRules = ref({
    serverUri: [{ required: true, message: '链接地址不能为空', trigger: 'blur' }]
});
