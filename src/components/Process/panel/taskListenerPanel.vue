...
<script setup lang="ts" name="taskListenerPanel">
...
const resetListenersList = () => {
  bpmnElementListeners.value = modelerStore.element?.businessObject?.extensionElements?.values?.filter((ex: any) => ex.$type === `flowable:TaskListener`) ?? [];
...
const saveSystemListener = () => {
  if (checkedListenerData.value.length > 0) {
    checkedListenerData.value.forEach((value: any) => {
      ...
      otherExtensionList.value = modelerStore.element.businessObject?.extensionElements?.values?.filter((ex: any) => ex.$type !== `flowable:TaskListener`) ?? [];
